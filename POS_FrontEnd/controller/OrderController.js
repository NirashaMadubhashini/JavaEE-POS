function generateOrderID() {
    ...
    $.ajax({
        url: "http://localhost:8085/backEnd/orders?option=GETID",
        method: "GET",
...
$("#btnSubmitOrder").click(function () {
    ...
    if (parseInt($("#subtotal").text()) > parseInt($("#txtCash").val())){
        ...
        if ($("#txtCash").val() == '') {
            ...
            $.ajax({
                url: "http://localhost:8085/backEnd/orders",
                method: "POST",
...
function bindOrderDetailsClickEvent(){
    $("#orderTable > tr").click('click', function () {
        ...
        $.ajax({
            url: "http://localhost:8085/backEnd/orders?option=SEARCH&orderId=" + oid,
            method: "GET",
            success: function (resp) {
